import type p5 from "../../types/p5types";
...
export function sketch0(p5: p5, options?: any) {
  const canvasSize = { width: options?.canvasWidth || 400, height: options?.canvasHeight || 400 };

  // a crude class for trailing behind the original walker
  class Trailer extends P5Node {
    prevStates: { x: number, y: number,  w: number, h: number }[];
    delay: number;
    parent?: Walker; // TODO use this for update??
    constructor(params: NodeParams & { delay: number, parent?: Walker }) {
      super(params);
      this.prevStates = [];
